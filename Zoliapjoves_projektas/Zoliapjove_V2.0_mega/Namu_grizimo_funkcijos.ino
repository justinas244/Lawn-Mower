...
void Laido_sekimas(bool reikalingas_pametimas) {
  ...
  while (true) {
    ...
    if (namo_mygtukas == 1 || krovimo_srove >= 0.35) {
      motor(80, 80);
      delay(500);
      motor(0, 0);
      peilio_apsukos(1000);
    ...
    if (now - kiek_laiko_jau_aptikes >= 8000) {
      pametimo_extra = 3000;
      pametimo_timeout = 3000;
    }
    else {
      pametimo_extra = 0;
      pametimo_timeout = 8000;
...
void laido_radimas() {
  ...
  while (true) {
    ...
    if (desinesP >= namu_aptikimo_riba && kairesP >= namu_aptikimo_riba ) {
      ...
      lcd.clear();
      lcd.setCursor(0, 0);
      lcd.print("Aptikti abu!!");
      delay(5000);
      if (desinesP >= sekimo_DP || kairesP >= sekimo_KP) {
        motor(sekimo_greitis, sekimo_greitis);
        delay(100);
        motor(sekimo_greitis + 30, -sekimo_greitis);
        delay(800);
        ivaziavimas_laidu(1);
        break;
      }
    ...
    if (desinesP >= namu_aptikimo_riba) {
      //letejimas(greitis, 0, 2, 1);
      motor(0, 0);
      aptiktas_laidas = 1;
      siuntimas_nameliui(3, 3);
